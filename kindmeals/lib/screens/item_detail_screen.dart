...
class ItemDetailScreen extends StatefulWidget {
  final Map<String, String> request;
  final Function(String) onDonationAccepted;
  final String? recipientId; // Add recipient ID parameter

  const ItemDetailScreen({
    super.key,
    required this.request,
    required this.onDonationAccepted,
    required this.recipientId, // Make it required
  });
...
class _ItemDetailScreenState extends State<ItemDetailScreen> {
  ...
  Future<void> _acceptDonation(bool needsVolunteer) async {
    try {
      final donationId = widget.request['id'];
      if (donationId != null && widget.recipientId != null) {
        // Add recipientId parameter to match API expectations
        await ApiService.acceptDonation(donationId, widget.recipientId!, needsVolunteer);

        // Remove donation from live requests
        widget.onDonationAccepted(donationId);
      } else {
        throw Exception('Missing donation ID or recipient ID');
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
