...
function Signup() {
  ...
  const handleSignup = async (event) => {
    ...
    try {
      const firebaseResult = await registerWithFirebase(email, password);
      ...
      // If Firebase registration is successful, continue with your backend registration
      const formData = new FormData();
      ...
      formData.append("about", about);
      formData.append("identificationDoc", identificationDoc);
      formData.append("firebaseUid", firebaseResult.user.uid);
      ...
      if (data.success) {
        ...
        // Don't login or navigate immediately - wait for email verification
      } else {
        setError(data.message || "Signup failed. Please try again.");
      }
    } catch (error) {
      console.error("Error signing up:", error);
      setError("Error during signup. Please try again. Make sure you're using a unique email address.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (verificationSent) {
    return (
      <div className="signup-container">
        <div className="signup-form verification-message">
