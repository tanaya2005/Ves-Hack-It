...
// src/Pages/LiveDonations/LiveDonations.jsx
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import './livedonations.css';
...
const LiveDonations = () => {
    ...
    const [loading, setLoading] = useState(true);
    const [acceptingDonation, setAcceptingDonation] = useState(null);
    const { user } = useAuth(); // Using the auth context
    ...
    const handleAccept = async (donationId) => {
        // First confirmation for accepting donation
        if (!window.confirm('Are you sure you want to accept this donation?')) {
            return;
        }

        // Second confirmation for volunteer requirement
        const needsVolunteer = window.confirm('Do you need a volunteer to help with pickup/delivery?');

        setAcceptingDonation(donationId);
        try {
            const response = await axios.post('http://localhost:4000/api/donations/accept', {
                donationId,
                needsVolunteer // Add this to your request body
            });
            ...
            if (response.data.success) {
                ...
                alert(`Donation accepted successfully! ${needsVolunteer ? 'A volunteer will be notified to help you.' : ''}`);
            } else {
                alert('Failed to accept donation. Please try again.');
            }
        } catch (error) {
                    ...
                    {loading ? (
                        ...
                        sortedDonations.map((item) => (
                            <div key={item._id} className="donation-card">
                                ...
                                {/* Only show action buttons for Recipients */}
